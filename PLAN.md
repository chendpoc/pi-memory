...
## Phase 9 — Long-Term Memory Consolidation ✅

Write → Consolidate → Retrieve loop for reducing cross-session forgetting.

- **Active-branch session loading** — shared `.jsonl` parser follows `id/parentId` from the active leaf and ignores non-active branch turns. Trainer and FTS index reuse the same parser.
- **Consolidation config** — `memory.json` supports `consolidation.enabled`, `min_user_turns`, MEMORY.md cap, Phase2 top-N, `max_unused_days`, and daily schedule defaults.
- **Write path** — `session_shutdown` only enqueues session metadata into `memories.sqlite`; no LLM work runs during shutdown.
- **Stage1** — pending sessions drain into stage1 rows with scope metadata; forked child sessions use delta extraction and clone sessions are skipped.
- **Phase2** — append+dedup updates global/project MEMORY.md files, writes backups, syncs workspace summaries, migrates project-scoped lines, and prunes stale exact-match lines conservatively.
- **Retrieve** — session-level MEMORY.md cap is injected as private context; preflight now greps scoped MEMORY.md in parallel with graph query.
- **Tools** — `memory_append` is lowered in prompt priority and queues explicit memories into stage1 instead of direct MEMORY.md writes when consolidation is configured.
- **CLI / observability** — `memory-status`, `consolidate`, `setup-schedule`, and `/memory --verbose` expose queue, index, scheduler, and recent log state.
